...
onMounted(() => {
  newsStore.getCategories().then(() => {
    const idx = displayCategories.value.findIndex(cat => cat.id === newsStore.currentCategory)
    if (idx !== -1) {
      activeTab.value = idx
    } else {
      activeTab.value = 0
...
watch(activeTab, (newVal) => {
  if (!newsStore.categories.length) return
  const cat = newsStore.categories[newVal]
  if (cat) newsStore.changeCategory(cat.id)
})
